<script lang="ts">
	...
	import { Alert, Button, Card, Icon, IDCardCanvas, Input, Label } from '$lib/components';
	...
	// it, so a background upload's re-read never discards unsaved layout work.
	// svelte-ignore state_referenced_locally
	const seed = data.template;
	let name = $state(seed.name);
	let subject = $state<Subject>(seed.subject as Subject);
	...
	async function save() {
		...
		try {
			...
			if (result.type === 'success') {
				...
				errorMessage = actionMessage(result, 'Could not save that template.');
...
<svelte:head><title>{name} — ID cards — Muallim</title></svelte:head>

<div class="mx-auto w-full max-w-[1400px] px-4 py-6 sm:px-6">
	<div class="mb-5 flex flex-wrap items-center justify-between gap-3">
		<div class="flex items-center gap-3">
			<Button href={resolve('/teach/id-cards')} variant="ghost" size="sm">
				<Icon icon={ArrowLeft01Icon} class="size-4" />
				All templates
			</Button>
			<h1 class="text-lg font-semibold">ID-card designer</h1>
		</div>
	...
	<div class="grid grid-cols-1 gap-6 lg:grid-cols-[260px_1fr_300px]">
		...
		<aside class="flex flex-col gap-5">
			<Card class="flex flex-col gap-3">
				...
				<Input id="template-name" bind:value={name} maxlength={120} />

				<Label for="subject">For</Label>
				<div class="flex gap-2" id="subject">
					{#each ['student', 'staff'] as const as s (s)}
						<Button
		...
		<div class="min-w-0">
			<div class="mx-auto w-full max-w-[420px]">
				...
				>
					{#if bgSrc}
						<img class="idedit__bg" src={bgSrc} alt="" aria-hidden="true" />
					{/if}
					<div class="idedit__bar" style:background-color={accent}></div>

					{#each layout as el (el.id)}
						{@const spec = kindSpec(el.kind)}
						<div
							class="idedit__el"
							class:idedit__el--selected={el.id === selectedId}
							class:idedit__el--photo={spec.photo}
							style:left="{el.x * 100}%"
							style:top="{el.y * 100}%"
							...
							style:border-color={spec.photo ? accent : undefined}
							role="button"
							tabindex="0"
							aria-label="{spec.label}: {spec.photo ? 'photo' : displayText(el)}"
							use:draggable={{
								stop: false,
						...
						>
							{#if spec.photo}
								<span class="idedit__photolabel">Photo</span>
							{:else}
								{displayText(el)}
							{/if}
							{#if el.id === selectedId}
								<span
									class="idedit__handle"
									aria-hidden="true"
		...
		<aside>
				...
				{#if selected}
					...
					{#if spec.photo}
						<p class="text-muted text-sm">
							Shows the person's photo on a printed card. The editor previews a placeholder box.
						</p>
					{:else if spec.authored}
						<div>
					...
					{:else}
						<p class="text-muted text-sm">
							Shows the person's own {spec.label.toLowerCase()} on a printed card. The editor previews
							“{spec.sample}”.
						</p>
					{/if}
					...
					{:else}
						<div>
							<Label for="el-color">Border colour</Label>
							<input
								id="el-color"
								type="color"
								class="mt-1 h-9 w-full cursor-pointer rounded-md border border-[var(--color-border-control)]"
			...
			<Card class="mt-5">
				...
				<div class="mx-auto w-full max-w-[240px]">
					<IDCardCanvas {orientation} {layout} {accent} {backgroundColor} {backgroundUrl} />
				</div>
			</Card>
		</aside>
	</div>
