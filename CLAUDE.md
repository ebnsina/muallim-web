...
- **One claim per section.** `Showcase` — an index number, an eyebrow, a heading, a hairline list, and the shot bleeding out to the window. Four of them: author, assess and mark, learn, sell. What *else* ships is a dense three-column index (`Capabilities`), never six cards of bullets.
- **Dark hero, light body.** The hero is a full-bleed dark aurora band (`.hero-blue` + `AuroraBackdrop` + `.grain`), fixed-dark in either theme; the rest follows the app theme, **light by default** — do not lock it dark. Light screenshots get `dark:brightness-90` so they do not glare on the dark page.
- **Header floats, then leaves.** The landing has its own `Nav` (absolute over the hero, scrolls away, no border) because the shared `MarketingHeader` — still used by `/solutions/[slug]` — overflows below ~360px. Buttons are pills (`Button pill`); the app keeps `rounded-control`.
- **Honesty is the voice, and it is on the page.** `ROADMAP` in `content.ts` says what is *not* built (mobile apps, SCORM/LTI, subscriptions), and the sell section says plainly that the Stripe driver refuses to run until its keys are set. Solution pages keep the same "works today" vs "on the roadmap" split. Section eyebrows are text, not `Badge`s.
