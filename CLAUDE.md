...
**Certificates: `/teach/certificates` withdraws by number.** muallim-api lists no issued certificates — `GET /v1/certificates/{serial}` answers one number at a time and nothing enumerates them — so the page looks a certificate up by its serial, shows what it actually says (learner, course, date, and whether it already stands withdrawn), and offers the withdrawal there, with a reason. It is not a delete: the number keeps answering, and from then on it answers that the certificate was withdrawn. The copy says exactly that, and the confirmation is a swap of buttons in a row whose caution line was already there — so asking moves nothing.
