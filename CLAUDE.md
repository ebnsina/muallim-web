...
- **`/v2` is the rebuilt landing, and it shows the product.** `src/lib/features/marketing-v2`: a dark hero over a real screenshot of `/dashboard`, then four `Showcase` sections — each one claim, its own hairline list, and a real screenshot bleeding out to the window (`ProductShot`, the frame with the address bar). Every image in `static/marketing/` is a genuine capture of the running app, taken at a 1180px viewport so its UI is legible at half size — never a drawing of the UI, never invented data. What ships is a dense index (`Capabilities`), not six cards of bullets; what does not is `ROADMAP`, on the page. When it replaces `/`, delete `src/routes/v2` and the old landing together.
