...
- **No pricing page, and that is deliberate.** The plans that used to sit here were invented numbers for a tenant-billing feature that does not exist. `commerce` is a workspace selling _its_ courses, which is a different thing. Do not put a price back on the site until there is one.
...
**Theme is a three-way choice, and Auto keeps following.** `$lib/theme.svelte.ts` holds it: `system` | `light` | `dark`, where `system` is stored as the _absence_ of the key — which is exactly what the boot script in `app.html` reads, so the two agree without sharing code. The media query is _listened to_, not read once: a reader on Auto whose Mac turns dark at sunset turns dark too. `ThemeToggle` renders either a cycling icon button (app header) or a segmented Auto/Light/Dark control (`segmented`, in the footer and in settings).
...
- **`/teach/payments`** — the gateways. **Credentials are write-only**: `PUT /v1/billing/credentials` takes them and _nothing_ reads one back, so the page shows only whether a gateway is connected (`GET /v1/billing/account?gateway=…`) and never a stored value, masked or otherwise. bKash's three secrets are packed by the web into one JSON string (`packBkashSecret`, `src/lib/billing.ts`) because that is the one field the API seals. Stripe onboards instead of taking keys.
...
`/people` is the workspace's members and its invitations, on one page, for whoever holds `user:manage` — `canManagePeople` in `$lib/roles.ts` (owner, admin) draws the account-menu link, and, as with `canAuthor`, it is a courtesy: muallim-api refuses anyone else and the page renders the refusal it gets.

- **The rules live in the API, and the page does not restate them.** Nobody may change their own role, a workspace never loses its last owner, only an owner may invite an owner. Every one of those is a 4xx with a sentence in it (`You cannot change your own role or remove yourself.`, `A workspace must keep at least one owner.`, `Your role does not permit this action.`), and the page shows the sentence in an `Alert`. Disabling the control instead would be a second copy of a rule that only the server can enforce — and a copy that is wrong the day the rule moves.
- **A refused role change puts the picker back.** The role `Select` PATCHes on change; on a failure the page reassigns the whole `roles` map from the row the server last sent. A control left displaying a change the API rejected is a lie about the workspace.
- **The invitation link is never shown.** `POST /v1/invitations` does not return the token — it goes to the invited address and nowhere else, which is what makes accepting it proof that the person holds that inbox. The page therefore offers no link to copy, and says why.
- **Removal ends a membership, not an account.** The copy says so, because "Remove" reads like a deletion and is not one.
- **Names repeat; addresses do not.** Every row action's `aria-label` carries the email, or a workspace with two people called Fatima has two buttons that say the same thing.
- **A long list ends in a button, not a cliff.** Both lists are keyset-paginated (`limit`, opaque `cursor`, `next_cursor`, `has_more`). "Load more" posts the cursor to an action, appends the page it gets back, and **does not `update()`** — re-running the load would throw away every page but the first. It disappears when `has_more` is false. There is no total, and there will not be one: muallim-api runs no `COUNT(*)`, so a page that printed "44 of 120" would be inventing the 120.
- **A write updates its row where it stands.** Same reason: a role change, a removal, a withdrawal and an invitation all reassign the `Paged` list in `$lib/paging.ts` rather than re-reading it, so a reader three pages in stays three pages in. Every one of them uses what the API sent back — the invitation `POST` returns the row, and the withdrawal reads the certificate back — never a guess at what the server now holds.

**Certificates: `/teach/certificates` is the list.** It was a serial lookup and nothing else, because nothing enumerated them; `GET /v1/certificates` (course:write, newest first, same cursor) now does, so the list leads: every certificate the workspace has issued, its number, who earned it, which course, when, and whether it stands. Serial lookup survives as a secondary affordance — it narrows the list to one row, for a registrar holding the number, which beats paging to it. Withdrawal is not a delete: the number keeps answering, and from then on it answers that the certificate was withdrawn, and why. The reason is required (three layers, `revokeCertificateSchema`), and the confirmation is a row that slides open under the certificate — never `window.confirm`, which is blocked.
