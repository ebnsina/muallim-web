...
test.describe('content bank', () => {
	...
	test('an author saves a question to the bank and adds it back to the quiz', async ({
		...
		const question = page.getByRole('listitem').filter({ hasText: 'Which is compiled?' }).first();
		...
		const banked = page.getByRole('listitem').filter({ hasText: 'Which is compiled?' });
		...
		await page
			.getByRole('listitem')
			.filter({ hasText: 'Which is compiled?' })
			.filter({ has: page.getByRole('button', { name: 'Add to this quiz' }) })
			.getByRole('button', { name: 'Add to this quiz' })
			.click();

		await expect(page.getByText('Which is compiled?')).toHaveCount(3); // two in quiz, one in bank
	});
});
